...
BEGIN
    -- Crea la clave maestra necesaria para el cifrado.
...
BEGIN
    -- Crea el certificado usado para proteger los datos.
...
BEGIN
    -- Crea la clave simétrica usada para cifrar datos sensibles.
...
BEGIN
    -- Crea la tabla necesaria para la aplicación.
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'UX_AppUserState_Account_Key' AND object_id=OBJECT_ID(N'dbo.AppUserState'))
    -- Ejecuta la operación necesaria para este bloque.
...
BEGIN
    ...
    -- Prepara una variable para usarla en esta validación.
