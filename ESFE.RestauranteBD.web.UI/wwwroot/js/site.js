const ESFERestaurante = (() => {
    ...
    const chat={
        ...
        refreshSuggestions(){
            const box=document.querySelector('.chat-suggestions');if(!box)return;let items=['Dueno','Administrador'].includes(currentRole())?['Pedidos de hoy','Ver empleados','Horario','Pagos']:currentRole()==='Cocina'?['Pedidos pendientes','Preparación','Ayuda']:currentRole()==='Barra'?['Reservas de hoy','Pedidos','Horario']:currentRole()==='Delivery'?['Pedidos listos','Entregas','Horario']:['Recomiéndame algo','Reservar mesa','Pedido a domicilio','Mi pedido'];box.innerHTML=items.map(x=>`<button type="button" onclick="ESFERestaurante.chat.ask('${x.replace(/'/g,"\\'")}')">${x}</button>`).join('');
        },
        ...
        respond(text){
            const q=text.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');const role=currentRole();const owner=['Dueno','Administrador'].includes(role);let r='Puedo orientarte con el menú, pedidos, reservas, pagos, domicilio y funciones del sistema.';
    ...
    const ready={
        ...
        render(){
            ...
            const canDeliver=['Dueno','Administrador','Delivery'].includes(role);const canHandover=['Dueno','Barra'].includes(role);
        ...
        confirmCash(id){
            if(!['Dueno','Administrador','Delivery'].includes(currentRole())){toast('Solo Delivery o el administrador puede confirmar el efectivo.', 'error');return;}
        ...
        handoverCash(id){
            ...
            addNotification(`Efectivo de ${o.id} entregado al restaurante.`,null,{type:'pago',orderId:o.id,title:'Efectivo recibido por restaurante',detail:`Delivery entregó el efectivo de ${money(o.total)} correspondiente al pedido ${o.id}.`,roles:['Dueno','Administrador','Barra']});
            toast('Entrega de efectivo registrada.');this.render();
        },
        deliver(id){
            if(!['Dueno','Administrador','Delivery'].includes(currentRole())){toast('Solo Delivery o el administrador puede completar entregas.','error');return;}
            ...
            addNotification(`Pedido ${o.id} entregado.`,null,{type:'pedido',orderId:o.id,title:'Entrega completada',detail:`${o.customerName||o.customer||'Cliente'} recibió el pedido.`,roles:['Dueno','Administrador','Delivery']});
